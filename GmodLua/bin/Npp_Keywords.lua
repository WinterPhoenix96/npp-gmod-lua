...
concommand.Add("npp_keywords_generate", function(ply, cmd, args)
	...
	if ( SERVER ) then
		if file.Exists("client.txt", "DATA") then
			xside = file.Read( "client.txt", "DATA" )
			file.Delete("client.txt")
		end
	else
		if file.Exists("server.txt", "DATA") then
			xside = file.Read( "server.txt", "DATA" )
